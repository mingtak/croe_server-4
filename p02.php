...
$data = array();
$q = $DB->Query("SELECT * FROM ". $t . " ORDER BY {$t}No DESC LIMIT ".$_pageStart.",".$_dataPerPage.";");
while($a = $DB->Arrays($q))
{
  $data[] = array(
...
?>
  <li>
  <div id="in-photo-pic">
  <a href="#"><img src="<?php echo UPLOAD . $data[$i]['pic']?>" onclick="MM_showHideLayers('photo-detail','','show');s('<?php echo sizeof($data)?>', 'a<?php echo $i?>');" /></a>
  </div>
